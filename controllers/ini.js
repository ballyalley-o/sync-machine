    ...
    fs.readFile(paths.iniPath, 'utf8', async (err, data) => {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          let burnInValue = await iniLooper.iniSimUpdate(
            'burnIn',
            modifiedLine,
            line
          )
...
const customIni = async (req, res) => {
  if (USERPROFILE) {
    fs.readFile(paths.iniPath, 'utf8', (err, iniOne) => {
      ...
      const lines = iniOne.split('.')
      const parsedIni = parse(lines)

      const EXTRACT_VALUE = 'profile' // <- section to extract, default will extract all
      const configuration = extractBySectionObj(parsedIni, EXTRACT_VALUE)
...
const dynamicIni = async (req, res, next) => {
  ...
  const normalizedDynamicSection = await normalize.normalizeParam(
    dynamicSection,
    TARGETS
  )
  const normalized = normalize.switchParam(
    param,
    normalizedDynamicSection,
    slicedNumber
  )
